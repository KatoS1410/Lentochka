...
class StanzaProcessor:
    ...
    def run_dsmc_command(self, stanza_info: Dict[str, Any], start_time: datetime.datetime) -> int:
        """
        Выполняет команду DSMC из stanza_info['dsmc_command'], логирует stdout/stderr в dsmc-сессию
        и дожидается завершения. Возвращает код возврата процесса.
        """
    ...
    def process_stanza(self, stanza_info: Dict[str, Any]) -> bool:
        try:
            ...
            backup_dirs_value = ' '.join(paths)
            command = dsmc_command_template.format(
                dsmc_path=dsmc_path,
                backup_dirs=backup_dirs_value
            )
            ...
            if return_code == 0:
                ...
                try:
                    ...
                    self.lentochka_log.log_lentochka_error(
                        f"***** WARNING: PERMISSION ERROR IN PROCESS_STANZA {lentochka_status_path}: {e} ***** Skipping stanza.")
                    return False
